...
const travelOptions = [
  {
    ...
    time: '58 min',
    cost: '$2.50',
    details: 'Bus 12 from Bus Stop A to Bus Stop B, then walk 10 min to Destination',
    sortKey: 58,
  },
  {
    id: 2,
...
bot.on('callback_query', (callbackQuery) => {
  ...
  if (action === 'help') {
    ...
    bot.sendMessage(chatId, 'Would you like to share your location or enter your starting point manually?', {
      reply_markup: {
        inline_keyboard: [
    ...
    userState[chatId].stage = 'waitingForLocation';
...
bot.on('message', (msg) => {
  ...
  if (currentState.stage === 'waitingForLocation' && msg.location) {
    currentState.data.start = `Lat: ${msg.location.latitude}, Lon: ${msg.location.longitude}`;
    currentState.stage = 'planningRouteEnd';
    bot.sendMessage(chatId, 'Please enter your destination:');
  } else if (currentState.stage === 'planningRouteStart') {
