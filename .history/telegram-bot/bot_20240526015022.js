...
const travelOptions = [
  ...
  },
  {
    ...
    time: '1 hr 9 min🚌',
    cost: '$1.80.',
    details: 'Walk 8 min, (550m) to Essec Business School, Bus 14 from to Aft Bras Basah Station A 31min (22 Stops), then walk 4 min (290m) to SMU',
    sortKey: 69,
  },
];
...
function sendMainMenu(chatId) {
  bot.sendMessage(chatId, 'Welcome to Pathfinder, our Public Transport Bot! How can I assist you today?', {
    reply_markup: {
      inline_keyboard: [
  ...
  if (option) {
    bot.sendMessage(chatId, `Details for Option ${option.id}:\nTime: ${option.time}\nCost: ${option.cost}\nDetails: ${option.details}`, {
      reply_markup: {
        ...
        inline_keyboard: [
        [{ text: 'View Route on Google Maps', url: option.url }],
...
bot.onText(/\/start/, (msg) => {
    const chatId = msg.chat.id;
    sendMainMenu(chatId); // Send the main menu when /start is clicked
    initializeUserState(chatId);
});
